...
**狀態：設計改造中 — 從手繪風格改成 Apple 風格用於功能測試**

### 設計改造計劃

**原因**：手繪馬克筆風格難以測試功能。先改成 Apple 風格（簡潔白底、系統字體）驗證功能，測試完可隨時切回原設計（`design/sketch-marker` 分支）。

**改動方向**：Apple 風格（系統字體、白底、簡潔灰色按鈕、無手繪邊框）
...
**全站樣式改動：**
- [ ] 移除所有 rough.js 手繪元素（RoughBox、RoughButton、RoughInput、RoughFrame）
- [ ] CSS 改動：移除網格背景、移除紅色品牌色、改用灰階
- [ ] 標題字體：Noto Serif SC → 系統字體（-apple-system, BlinkMacSystemFont...）
- [ ] 按鈕/輸入框：白底 + 淺灰邊 + 圓角 8px
- [ ] 顏色系統：背景 #fff、文字 #000、次文字 #666、邊框 #d5d5d7

**結果頁（Result Page）改動：**
- [ ] 歷史列表只顯示前 3 次記錄 + 「查看更多」按鈕（非「清空紀錄」）
- [ ] 底部只保留 2 個主按鈕：「繼續摸魚」| 「分享」
- [ ] 「清空紀錄」按鈕移到 History 頁面

**老闆鍵（假 Outlook 郵件頁）改動：**
- [ ] 「老闆走了」按鈕改放在左上角，跟「新增郵件」格式完全一樣
- [ ] 這樣看起來像郵件應用的正常功能，不可疑

**英文版本：**
- [ ] 「繼續摸魚」→ "Continue" / "Keep Going"
- [ ] 「分享」→ "Share"
- [ ] 「清空紀錄」→ "Clear History"
...
- [ ] **Milestone 2：Daily Reset & History**
  - 24 小時自動重置（午夜 12:00）
  - localStorage 存儲和讀取正常
  - 歷史紀錄頁面展示正確
...
- [ ] **Milestone 4：響應式 & 跨機型測試**
  - 響應式設計：手機 ≤520px、桌機 >520px
  - Chrome、Safari、iOS Safari、Android Chrome 最新版本
  - 無視覺 bug
...
| 步驟 | 項目 | 狀態 |
|------|------|------|
| 1 | PRD（產品需求） | ✅ 完成（`on-the-clock/PRD.md`） |
| 2 | 選擇框架 | ✅ React + Vite + TypeScript |
| 3 | GitHub 版本控制 | ✅ 完成（SSH 已設置） |
| 4 | 小步迭代 + 經常測試 | 🔄 進行中 |
| 5 | Vercel 部署 | ⏳ 待做 |
